"""Prepare or explicitly execute the isolated Stage 5C pre-production run."""
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=PILOT5C_CONFIG)
    parser.add_argument("--historical-root", type=Path)
    parser.add_argument("--output-dir", type=Path, default=PILOT5C_OUTPUT_DIR)
    parser.add_argument("--run-live", action="store_true")
    parser.add_argument("--api-key-environment", default="OPENAI_API_KEY")
    parser.add_argument("--timeout-seconds", type=float, default=120.0)
    args = parser.parse_args()

    _, _, preflight, _ = prepare_pilot5c(
    ...
    if args.run_live:
        ...
        _, report, paths = run_live_pilot5c(
            provider=provider,
            config_path=args.config,
            historical_root=args.historical_root,
            output_dir=args.output_dir,
