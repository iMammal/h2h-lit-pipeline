...
def test_responses_provider_selects_pilot5d_schema_explicitly():
    output = json.dumps({"proposal": "preserved"})
    session = FakeSession(
        FakeResponse(
            {
    ...
    provider.generate(
        ...
        prompt="prompt",
        input_snapshot={"text": "record"},
        parameters={"response_schema_version": "1.3.0"},
        request_id="request:5d",
        attempt_number=1,
    )

    _, kwargs = session.calls[0]
    assert kwargs["json"]["text"]["format"]["schema"] == pilot5d_response_schema()
